    ...
    .balign 8
enter_trap_asm:
    csrrw x31, sscratch, x31 # 暂时交换保存一下x31，x31此时保存内核态的satp，因为之前sscratch保存的就是内核态的satp，参见restore_trap_asm
    csrrw x31, satp, x31 # 交换，x31保存用户态的satp（可以丢弃, Task中有）, satp保存内核态的satp
    sfence.vma
...
restore_trap_asm:
    ...
    csrrw x31, sscratch, x31 # 交换, x31保存用户态satp, sscratch保存x31最终的值
    csrrw x31, satp, x31 # 交换，x31保存内核态的satp
    sfence.vma
    csrrw x31, sscratch, x31 # 恢复x31，sscratch还是保存着内核态的satp
    sret
init_trap_entry_asm:
...
trap_context_asm: # 保存TrapContextStore的全局变量
    .space 8*34 #注意size要和TrapContextStore一致
