    ...
    .balign 8
enter_trap_asm:
    csrrw x31, sscratch, x31 # 暂时交换保存一下x31，拿到内核栈
    ...
    sd x31, 33*8(x31) #把自己也保存一下，这个位置存内核栈，好用于恢复

    mv x2, x31# sp=x2
    # fn (&TrapContextStore) &TrapContextStroe
    mv x10, x31 # 设置参数用于消费上下文
    call trap_entry #这里返回x10=TrapContext
    call restore_trap_asm 
restore_trap_asm:
    # fn (ptr *TrapContextStore)
    # a0=x10=ptr
    mv x31, x10 # t6=x31
    ld x30, 33*8(x31) #t5=x30
    csrw sscratch, x30 #保存内核栈指针
    ld x30, 32*8(x31) 
    csrw sepc, x30
